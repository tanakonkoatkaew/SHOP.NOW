// Shared OAuth buttons — each keeps its brand colour so it stays instantly
// recognisable. The logos are white (fill-current) on the coloured buttons,
// and full-colour on Google's white button, per each brand's guidelines.

const base =
  'w-full flex items-center justify-center gap-2.5 py-3.5 font-bold rounded-xl ' +
  'text-sm cursor-pointer transition-all shadow-lg'

export function DiscordButton({ label, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`${base} bg-[#5865F2] hover:bg-[#4752C4] text-white shadow-[#5865F2]/20 hover:shadow-[#5865F2]/35`}
    >
...
export function GoogleButton({ label, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`${base} bg-amber-500 hover:bg-amber-600 text-white shadow-amber-500/20 hover:shadow-amber-500/35`}
    >
      {/* The 4 quadrants of the G mark share one fill so it reads as a solid
          white logo on the amber button, matching Discord/Facebook. */}
      <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
export function FacebookButton({ label, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`${base} bg-[#1877F2] hover:bg-[#0C63D4] text-white shadow-[#1877F2]/20 hover:shadow-[#1877F2]/35`}
    >
      <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
